...
async function handleSee( args, ctx, task ) {
	let [ selector, frame ] = args.split( '@' )
	let options = _waitOptions

	options.visible = true
	selector = lookupSelector( selector )
	if ( frame ) {
		frame = lookupFrame( frame )
		frame = await _page.frames().find( f => f.name().match( frame ) )
		await frame.waitForSelector( selector, options )
	} else {
		await _page.waitForSelector( selector, options )
	}
}
...
async function handleType( args, ctx, task ) {
	...
	const selectorAndIframe = parts.shift()

	let [ selector, frame ] = selectorAndIframe.split( '@' )
