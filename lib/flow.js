...
let _browser, _page, _config, _waitOptions, _references
...
function run( config ) {
	...
	_references = flatten( Object.assign( {}, _config.references ) )
...
function runFlow( flow ) {
	...
	_config.flows[ flow ].forEach( step => {
		steps.push( {
			title: step,
			task: async ( ctx, task ) => {
...
async function handleGoto( object, args ) {
...
async function handleSeeCSS( selector, args ) {
...
async function handleSeeXPath( selector, args ) {
	const options = Object.assign( { visible: true }, _waitOptions, args )
	const frame = await getSelectorFrame( selector )
...
async function handleType( object, args ) {
	const selector = objectToSelector( object )
...
async function handleSelect( object ) {
	...
	let values = object.values.type === 'Literal'
		? object.values.value
		: lookupReference( object.values.name )

	if ( typeof values === 'string' ) {
		values = values.split( ',' ).map( v => v.trim() ).filter( Boolean )
	}
	if ( ! Array.isArray( values ) ) {
		throw new Error( `expect values to select to be string or array` )
	}

	await frame.select( selector.str, ...values )
}

async function handlePress( object, args ) {
...
async function handleSaveScreenshot( object, args ) {
...
async function handleSavePage( object, args ) {
...
async function handleSaveCookies( object ) {
	const dst = object.destination.value
...
async function handleWait( object, args, ctx, task ) {
	switch ( object.value ) {
		...
		case 'url':
			...
			break
		case 'request':
			await handleWaitRequest( object, args, ctx, task )
			break;
		case 'response':
			await handleWaitResponse( object, args, ctx, task )
			break;
		default:
...
async function handleWaitPage( object, args ) {
...
async function handleWaitUrl( object, args ) {
...
async function handleWaitRequest( object, args ) {
	const url = buildUrl( getWaitConjunction( args, 'to' ) )
	const options = Object.assign( _waitOptions, args )

	await _page.waitForRequest( url, options )
}

async function handleWaitResponse( object, args ) {
	const url = buildUrl( getWaitConjunction( args, 'from' ) )
	const options = Object.assign( _waitOptions, args )

	await _page.waitForResponse( url, options )
}
...
async function handleRemember( object, args, ctx, task ) {
	switch ( object.type ) {
		case 'ReserveObject':
			await handleRememberUrlPart( object, args, ctx, task )
			break
		case 'Literal':
...
async function handleRememberUrlPart( object, args ) {
	...
	switch ( part.toLowerCase() ) {
		...
		case 'qs':
			...
			break;
		case 'path':
			paths = urlParts.pathname.split( '/' ).filter( v => !!v )
...
async function handleRememberSelectorPart( object, args ) {
	const selector = objectToSelector( object )
	const frame = await getSelectorFrame( selector )
